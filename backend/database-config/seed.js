    ...
    try {
        ...
        const seedCollection = async (collectionName, data) => {
            ...
            if (count === 0) {
                ...
                console.log(`Inserted ${collectionName}:`, result.insertedIds); // eslint-disable-line no-console
            } else {
                console.log(`${collectionName} already has data, skipping seed`); // eslint-disable-line no-console
            }
        };
        ...
        console.error("Error seeding database:", error.message || error); // eslint-disable-line no-console
    } finally {
        await client.close();
    }
};
