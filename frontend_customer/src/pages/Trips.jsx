...
useEffect(() => {
    ...
    axios.get(`/trips?customer_id=${id}`,{headers: {'x-access-token': `${token}`}})
      .then((response) => {
      ...
      .catch((error) => {
        if (error.response) {
          console.error('Backend responded with an error:', error.response.status);
...
/*
  useEffect(() => {
    Promise.all([fetch(`/trips?customer_id=${id}`, { headers: { 'x-access-token': `${token}` } })])
      .then(([tripsRes]) =>
